...
- A179 (branch refactor/code-quality-2026-06-10) - **Code-quality pass: lint 73 problems -> 0, lint gated, component/logic separation, hook correctness.** Executed on a separate branch + worktree so main and the running app were never touched. (1) Server mechanical: underscore unused-vars convention in eslint config; dead assignments removed; { cause } on re-thrown errors; desktopAlert control-char regex -> charCode filter. (2) 28 react-refresh/only-export-components: pure logic moved verbatim to marketChartMarkers.ts / replayChartsData.ts / reviewEntryExitChartLogic.ts / estimatorClock.ts, importers + tests updated (MarketChart.test.ts renamed marketChartMarkers.test.ts). (3) React hook fixes with ZERO eslint-disables: a real refs-during-render bug in EstimatorSpxChart deleted (unobservable by analysis); SpxHeatmapPanel tf useMemo un-skips the React Compiler; set-state-in-effect resets -> render-time prev-guards across App.tsx (one provably dead effect deleted), LiveSpreadEstimatorPanel, FplIndicatorPanel, RrgPanel, SpxHeatmapPanel; useSpxMaContext cache-hit set moved to the promise path. (4) Gates: validate:mvp = typecheck && lint && test && build; CI adds npm run lint. Validation: eslint 0 repo-wide, 565/565 tests, build clean, browser smoke (worktree scratch :5190 with a full data copy) across all six surfaces - 0 console errors, 0 overflow. MERGE NOTES: renumber A179 if main consumed it; main's in-flight daily-pull-launcher changes touch server/dailySync.ts - expect a small, easy conflict there; re-run the smoke after merge.
