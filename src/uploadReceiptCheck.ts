...
export function buildUploadReceiptCheck(summary: DailySummary | null | undefined): UploadReceiptCheck | null {
  ...
  if (summary.uploadStatus === "payload_ready_unconfirmed") {
    return {
      badge: "Needs upload",
      detail: `${summary.date} has ${formatNumber(summary.payloadRows)} locally staged tracker rows across ${formatNumber(summary.uploadTabCount)} tab, but the Google tracker update is not confirmed yet.`,
      facts: compactFacts([
      ...
      steps: [
        `Configure one write-capable Google credential: ${GOOGLE_CREDENTIALS}.`,
        "Click Refresh Google in Source State, or run npm run google:snapshot from this app folder.",
        `Confirm SPX Spread Trade Tracker > Daily Sync Runs contains ${summary.date} with google_upload_status complete.`,
      ],
  ...
  return {
    badge: "Upload gap",
    detail: `${summary.date} does not have a confirmed compact Google tracker payload or tracker upload receipt.`,
    facts: compactFacts([
...
function payloadFact(summary: DailySummary): string {
  ...
  const tabLabel = summary.uploadTabCount === 1 ? "tab" : "tabs";
  return `${formatNumber(summary.payloadRows)} rows / ${formatNumber(summary.uploadTabCount)} ${tabLabel}`;
}
